...
//            ['role_info','required'],
    ...
    public function beforeSave($insert)
    {
        if($this->user_permission){
            $this->user_permission=implode(",",$this->user_permission);
        }
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        $upms_ids=explode(",",$this->user_permission);
        $upms_arr=UserPermission::find()->where(['id'=>$upms_ids])->asArray()->all();
        $manager->revokeAll($this->id);//对应下面的assign，assign之前先revokeAll
        $per_parent=$manager->getPermission("权限".$this->id);
        if(!$per_parent){
            $per_parent=$manager->createPermission("权限".$this->id);
            $manager->add($per_parent);
        }
//        $manager->revokeAll($this->id);
        $manager->removeChildren($per_parent);
        foreach ($upms_arr as $k=>$v){
            $per_child=$manager->getPermission($v['route']);
//            $manager->add($per_child);
            $manager->addChild($per_parent,$per_child);
        }
        $manager->assign($per_parent,$this->id);
    ...
    {
        $this->user_permission=explode(",",$this->user_permission);
